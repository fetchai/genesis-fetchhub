...
Wait until the blockchain reaches the target upgrade block height defined in the governance proposal. At that point, all nodes will halt. It is **\*expected\*** to see an upgrade-required error in the logs similar to:

```log
ERR UPGRADE "v0.15.0" NEEDED at height: <UPGRADE_HEIGHT>
ERR CONSENSUS FAILURE!!! err="UPGRADE \"v0.15.0\" NEEDED at height: <UPGRADE_HEIGHT>"
```
...
INF applying upgrade "v0.15.0..." at height: <UPGRADE_HEIGHT> module=x/upgrade
```
...
```log
INF finalized block ... height=<UPGRADE_HEIGHT> module=consensus ...
INF executed block ... height=<UPGRADE_HEIGHT> module=consensus
INF committed state ... height=<UPGRADE_HEIGHT> module=consensus
