...
function secp256k1UncompressedPubkeyToFetchAddress(pubKeyHexStr) {
    const binaryAddress = createHash('ripemd160').update(
        createHash('sha256').update(
            secp256k1.publicKeyConvert(Buffer.from('04' + pubKeyHexStr, 'hex'), true)
        ).digest()).digest()

    return bech32.encode('fetch', bech32.toWords(binaryAddress))
}

const staking = new Contract('Staking.json', '0x351baC612B50e87B46e4b10A282f632D41397DE2', 11061460, "latest");
        ...
        for (let i = 0; i < staking_event_names.length; ++i) {
            ...
            for (let i = 0; i < events.length; ++i) {
                ...
                if (e.returnValues.stakerAddress in users_dict) {
                    ...
                    userPubkey = await getPublicKeyFromTxHash(e.transactionHash);
